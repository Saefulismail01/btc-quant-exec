...
**Execution order is strict:** I-00 → I-08 → I-01 → I-02 → I-03.

---

### I-00 — HMM Regime Predictive Power Test ⚠️ PREREQUISITE

**Priority:** 🔴 CRITICAL | **Phase:** 1 — Must be first | **Effort:** Low
...
If yes → HMM has predictive power. Retain and optimize.
If no → Layer 1 is generating noise. Replace before building further.
...
2. Run HMM inference across the entire dataset to generate regime labels for each candle
...
Minimum bar for "HMM has predictive power":
...
- [ ] `backtest/hmm_predictive_power_test.py` runs standalone with no manual input
...
- [ ] Results are reviewed and one of two decisions is documented:
...
> **Dependency:** Run after I-08 (more historical data improves CVD signal quality).
...
> **Dependency:** Run after I-00 (HMM must pass existence test), I-01 (clean CVD data), and I-08 (sufficient historical data).
...
> **Dependency:** Should be completed before I-00, I-01, and I-03 — more data directly improves the reliability of all three tests.
...
Phase 1 (strict order):
    ...
    I-01  → Fix CVD per-candle (cleaner features for HMM and MLP)
...
Phase 3 (parallel, any time):
    I-07  → Heartbeat & alerting
```

> If I-00 returns **FAIL**: stop Phase 1 after I-08 and I-01. Redesign Layer 1 before proceeding. Document the replacement decision in `backtest/results/hmm_power_test_decision.md`.
...
- [ ] If I-00 returned FAIL — Layer 1 has been replaced with a model that passes the same test
