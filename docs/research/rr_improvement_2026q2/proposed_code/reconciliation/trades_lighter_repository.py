...
Design principles:
- All writes are idempotent (INSERT OR REPLACE / upsert semantics via DuckDB).
- No Lighter SDK import — only standard Python + duckdb.
- Receives a duckdb.DuckDBPyConnection; caller controls connection lifecycle.
- Logs context on every error (no secrets).

Table is created by migration 001_create_trades_lighter.sql.
"""
...
# ── DDL helper (used in tests with :memory: connection) ───────────────────────
...
def ensure_schema(conn: duckdb.DuckDBPyConnection) -> None:
    """Create tables if they don't exist. Safe for :memory: and file DBs."""
...
class TradesLighterRepository:
    """
    CRUD operations for the `trades_lighter` table.

    Usage:
        conn = duckdb.connect(":memory:")
        ensure_schema(conn)
        repo = TradesLighterRepository(conn)
    """
    ...
    def get_open_trade_ids(self) -> set[str]:
        """Return set of trade_ids currently flagged OPEN in DuckDB."""
        try:
            ...
            logger.error(
                "[TradesLighterRepo] get_open_trade_ids failed",
                extra={"error": str(exc)},
                exc_info=True,
            )
            raise
        ...
        try:
            row = self._conn.execute(
                ...
                SELECT trade_id, symbol, side, ts_open_ms, ts_close_ms,
                       entry_price, exit_price, size_base,
                       pnl_usdt, fee_usdt, status, exit_type,
                       last_synced_ms, source_checksum, reconciliation_lag_ms,
            ...
            logger.error(
                "[TradesLighterRepo] get_trade failed",
    ...
    def upsert_trade(self, trade: LighterTradeMirror) -> None:
        ...
        DuckDB does not support INSERT OR REPLACE for tables with constraints
        beyond PK, so we use explicit INSERT … ON CONFLICT DO UPDATE.
        """
        now = _NOW_MS()
        ...
        try:
            self._conn.execute(
                ...
                [
                    ...
                    trade.fee_usdt,
                    trade.status.value,
                    trade.exit_type.value if trade.exit_type else None,
                    trade.last_synced_ms,
                    trade.source_checksum,
                    trade.reconciliation_lag_ms,
                    trade.created_at_ms,
                    trade.updated_at_ms,
                ],
            )
            logger.debug(
                "[TradesLighterRepo] upserted trade",
                extra={"trade_id": trade.trade_id, "status": trade.status.value},
            )
        except Exception as exc:
            logger.error(
                "[TradesLighterRepo] upsert_trade failed",
                extra={"trade_id": trade.trade_id, "error": str(exc)},
                exc_info=True,
            )
            raise

    def upsert_from_open_position(self, pos: LighterPosition) -> None:
        """Insert or update an OPEN trade from a Lighter position snapshot."""
    ...
    def mark_closed(
        ...
        """
        Transition a trade from OPEN → CLOSED.

        Computes reconciliation_lag_ms: how long it took us to detect closure.
        Called by the reconciliation worker when a 'stuck OPEN' is found.
        """
        now = _NOW_MS()
        ...
        try:
            self._conn.execute(
                ...
                """,
                [
                    closed_order.ts_close_ms,
                    closed_order.exit_price,
                    closed_order.pnl_usdt,
                    closed_order.fee_usdt,
            ...
            logger.warning(
                "[TradesLighterRepo] reconciled stuck_open → CLOSED",
                extra={"trade_id": trade_id, "lag_ms": lag_ms},
            )
        except Exception as exc:
            logger.error(
                "[TradesLighterRepo] mark_closed failed",
                extra={"trade_id": trade_id, "error": str(exc)},
                exc_info=True,
            )
            raise
    ...
    def write_reconciliation_log(
        ...
        duration_ms: int,
        api_calls_count: int,
        api_throttled_count: int,
        errors: list[str],
        success: bool,
    ) -> None:
        """Persist one run summary to reconciliation_log."""
        ...
        try:
            self._conn.execute(
                ...
                INSERT INTO reconciliation_log (
                    log_id, ts_ms, mode,
                    stuck_resolved, missing_resolved, upserted, snapshots_inserted,
                    duration_ms, api_calls_count, api_throttled_count,
                    errors, success
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                [
            ...
            logger.error(
                "[TradesLighterRepo] write_reconciliation_log failed",
...
def _row_to_mirror(row: tuple) -> LighterTradeMirror:
    (
        trade_id, symbol, side, ts_open_ms, ts_close_ms,
        entry_price, exit_price, size_base,
        pnl_usdt, fee_usdt, status_str, exit_type_str,
        last_synced_ms, source_checksum, reconciliation_lag_ms,
        created_at_ms, updated_at_ms,
    ) = row
