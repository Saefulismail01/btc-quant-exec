"""Partial TP execution: 60% at TP1 then trail 40%."""
...
def simulate_trade(
    ...
    """Close 60% at TP1 and trail the remaining 40%."""
    ...
    for _, minute in window.iterrows():
        ...
        if not tp1_hit:
            ...
            if is_long and high >= tp1:
                tp1_hit = True
                tp1_fill = tp1
                trailing_sl = max(entry, close - ATR_MULT * atr)
                continue
            if (not is_long) and low <= tp1:
                tp1_hit = True
                tp1_fill = tp1
                trailing_sl = min(entry, close + ATR_MULT * atr)
                continue
            ...
            if is_long:
                ...
                if low <= trailing_sl:
                    exit_price = TP1_SIZE * tp1_fill + RUNNER_SIZE * trailing_sl
                    return {"exit_price": exit_price, "pnl_pct": _pnl_pct(entry, exit_price, side, size), "exit_type": "TRAIL", "holding_minutes": hold}
            else:
    ...
    close_next = _get_col(bar_next, ["Close", "close"], default=entry)
    if tp1_hit:
        exit_price = TP1_SIZE * tp1_fill + RUNNER_SIZE * close_next
    else:
