...
def run_true_walkforward(
    ...
    for i in range(t_start, t_end):
        ...
        if position is None:
            try:
                # L1: BCD Regime ─────────────────────────────────────────────
                # Single BOCPD pass: get_regime_and_states() jalankan BOCPD
                # satu kali, lalu fast-path reconstruct states dari _changepoints
                # cache. Ini O(N²) × 1 bukan × 2. BOCPD adalah bottleneck utama
                # (window 500 candle, tiap candle = 500² = 250k ops).
                # Double BOCPD menyebabkan ETA naik terus (250k×2 per candle).
                ...
                if spec.trade_gate in ("ACTIVE", "ADVISORY") and risk.can_trade:
                    ...
                    # Fixed 2% risk per trade — prevents leverage explosion
                    RISK_PCT    = 0.02
                    ...
                    position = {
                        ...
                        "lev"       : min(risk.approved_leverage, 10),  # hard cap 10x
