...
class LighterExecutionGateway(BaseExchangeExecutionGateway):
    ...
    def __init__(self):
        ...
        if self.execution_mode == "testnet":
            ...
            self.base_url = os.getenv(
                "LIGHTER_MAINNET_BASE_URL", "https://mainnet.zklighter.elliot.ai/api/v1"
            )
            self.ws_url = os.getenv(
    ...
    def _generate_auth_token(self) -> str:
        ...
        try:
            ...
            logger.error(
                f"[LIGHTER] SDK auth token generation failed: {e}. Auth will fail."
            )
            expiry_unix = int((datetime.utcnow() + timedelta(hours=8)).timestamp())
            random_hex = secrets.token_hex(16)
            return (
                f"{expiry_unix}:{self.account_index}:{self.api_key_index}:{random_hex}"
            )
        ...
        for attempt in range(self.RETRY_ATTEMPTS):
            try:
                ...
                if "nonce" in error_str or "sequence" in error_str:
                    ...
                    return OrderResult(
                        success=False, error_message=f"Nonce mismatch: {e}"
                    )
        ...
        try:
            ...
            btc_market = next(
                (
                    ...
                    for m in markets
                    if m.get("symbol") == "BTC"
                    and m.get("market_type", "perp") == "perp"
                ),
                None,
            )
            ...
            if btc_market:
                self._price_decimals = int(
                    btc_market.get(
                        "supported_price_decimals", self.DEFAULT_PRICE_DECIMALS
                    )
                )
                self._size_decimals = int(
                    btc_market.get(
                        "supported_size_decimals", self.DEFAULT_SIZE_DECIMALS
                    )
                )
    ...
    async def place_market_order(
        ...
        try:
            ...
            current_price = (
                float(details_list[0].get("last_trade_price", 0)) if details_list else 0
            )

            if current_price <= 0:
                return OrderResult(
                    success=False, error_message="Invalid current price from API"
                )
            ...
            order_result = await self._retry_call(
                lambda: self._submit_order(order_payload), f"Place {side} market order"
            )
    ...
    async def _submit_order(self, payload: Dict[str, Any]) -> OrderResult:
        ...
        try:
            ...
            if order_type == "MARKET":
                ...
                if (
                    resp
                    and hasattr(resp, "avg_execution_price")
                    and resp.avg_execution_price
                ):
                    actual_fill_price = unscale_price(
                        resp.avg_execution_price, self._price_decimals
                    )
                filled_price = (
                    actual_fill_price
                    if actual_fill_price and actual_fill_price > 0
                    else unscale_price(price_scaled, self._price_decimals)
                )
    ...
    async def place_sl_order(
        ...
        try:
            if not self.trading_enabled:
                return OrderResult(success=False, error_message="Trading is disabled")
            ...
            logger.info(
                f"[LIGHTER] ✅ SL order placed: {order_id} | {side} SL @ ${trigger_price:,.2f}"
            )
            return OrderResult(
                success=True,
                order_id=order_id,
    ...
    async def place_tp_order(
        ...
        try:
            ...
            SLIPPAGE_PCT = (
                0.003  # 0.3% buffer — tighter than SL since TP direction is favorable
            )
            if is_ask:  # SELL to close LONG
            ...
            logger.info(
                f"[LIGHTER] ✅ TP order placed: {order_id} | {side} TP @ ${trigger_price:,.2f}"
            )
            return OrderResult(
                success=True,
                order_id=order_id,
    ...
    async def fetch_last_closed_order(
        ...
        try:
            ...
            for last_order in closed_orders:
                ...
                if filled_price > 0:
                    # Filter by expected SL/TP prices if provided
                    # Compare exit price with SL (for SL hits) or TP (for TP hits)
                    if (expected_sl_price or expected_tp_price) and tolerance_pct > 0:
                        matches = False

                        if expected_sl_price:
                            sl_diff_pct = (
                                abs(filled_price - expected_sl_price)
                                / expected_sl_price
                                * 100
                            )
                        ...
                        if expected_tp_price and not matches:
                            tp_diff_pct = (
                                abs(filled_price - expected_tp_price)
                                / expected_tp_price
                                * 100
                            )
                            if tp_diff_pct <= tolerance_pct:
                                matches = True

                        if not matches:
                            logger.debug(
                                f"[LIGHTER] Skipping order {order_id}: exit ${filled_price:,.2f} "
                                f"does not match SL ${expected_sl_price:,.2f} or "
                                f"TP ${expected_tp_price:,.2f} (tolerance: {tolerance_pct}%)"
                            )
                            continue
                    ...
                    logger.info(
                        ...
                        f"@ ${filled_price:,.2f}"
                        + (
                            f" [matched SL ${expected_sl_price:,.2f} or TP ${expected_tp_price:,.2f}]"
                            if (expected_sl_price or expected_tp_price)
                            else ""
                        )
                    )
                    return {
        ...
        try:
            ...
            for order in orders.get("orders", []):
                ...
                if filled_quote > 0 and filled_base > 0:
                    ...
                    return filled_quote
                logger.warning(
                    f"[LIGHTER] Order {order_id} found but fill amounts zero"
                )
                return None
        except Exception as e:
    ...
    async def _fetch_account(self) -> Dict[str, Any]:
        ...
        data = await self._make_request(
            "GET", "/account", params={"by": "index", "value": str(self.account_index)}
        )
    ...
    async def get_open_position(self) -> Optional[PositionInfo]:
        ...
        try:
            ...
            # Find BTC position
            btc_position = next(
                (p for p in positions if p.get("market_id") == self.MARKET_ID), None
            )
            ...
            return PositionInfo(
                symbol=self.SYMBOL,
                side=side,
                entry_price=float(btc_position.get("entry_price", 0))
                / (10**self._price_decimals),
                quantity=quantity,
    ...
    async def close_position_market(self) -> OrderResult:
        ...
        try:
            ...
            if not position:
                return OrderResult(
                    success=False, error_message="No open position to close"
                )
            ...
            order_result = await self._retry_call(
                lambda: self._submit_order(payload), f"Close {position.side} position"
            )
    ...
    async def fetch_account_nonce(self) -> int:
        ...
        try:
            data = await self._make_request(
                "GET", "/nextNonce", params={"account_index": str(self.account_index)}
            )
